<template>
	<view class='goods-wrapper'>
		<view class="spuInfoImg">
			<text class="current">{{goodsIndex}}/{{goods_image.length}}</text>
			<swiper class="swiper" @change="scrollImagesChange" :indicator-dots="false" :interval="3000" :duration="1000">
				<swiper-item
				v-for = "(item,index) in goods_image" 
				:key="index">
		...
		<view class="spuInfo">
			<view class="spuPrice">
				<image class="spuPriceBg" src="../../static/shareBottomBg.png" mode=""></image>
				<view class="price">
					<text class="salePrice">{{goods.price}}</text>
					<text class="storePrice">¥{{goods.productprice}}</text>
					<text class="pin-type">{{pinInfo.pin_count}}人团</text>
				</view>
			</view>
			<view class="spuDetailInfo">
				...
				<view class="saleNum">
					...
					<view class="stock">
						已售
						<text class="remaining">{{goods.seller_count}}</text>
						件
					</view>
				</view>
			</view>
			...
			<view class="pinRecords buyRecords">
				<view class="pinIntro" @click="seeRule">
					<view class="i-flex i-flex-spb pinIntro-top">
						<view class="pinIntro-title">
							拼团玩法
						</view>
						<view class="">
							查看规则>
			...
			<view class="spuDetail">
				<image class="goodsDetailTitleImg" src="../../static/goodsDetailTitle.png" role="img"></image>
				<!-- 商品详情 -->
				<view class="warmPrompt">
					<view class="wxParse">
						<view class="" v-html="goods.description">
			...
			<image class="sloganImg" src="../../static/icon-index-slogan.png" mode=""></image>
		</view>
		<!-- 底部按钮 -->
			...
			<view class="goods-bottom-bar">
				<view class="leftBtn">
					<navigator url="/pages/index/index">
						<view class="bar-item back-to-index">
							<image class="icon" src="../../static/icon-tab-index.png" mode=""></image>
							回首页
						</view>
					</navigator>
				...
				<view class="gobuy">
					<label class="balanceBtn">
						<view class="bar-item balance">
							<view class="bar-item-price">¥600.00</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			get_goods_details: function(id){
				...
				util.request({
				      ...
				      success: function (res) {
						...
						that.goods_image = res.data.data.goods_image;
						that.pinInfo = res.data.data.pin_info;
						},
						fail: (err) => {
							console.log(err)
						}
			...
			seeRule: function(){
				uni.navigateTo({
					url:"../pintuanrule/pintuanrule?type=pintuan"
				})
			}
		}
	}
...
.spuInfo .spuDetailInfo, .spuInfo .straightInLive, 
.spuInfo .buyRecords {
  width: 670rpx;
  background: #fff;
